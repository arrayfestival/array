$(function () {
  ...
  if ($('html').hasClass('js')) {
    ...
    gsap.to('.logo-main', {duration: 1.5, opacity: 1, delay: 4})
    var $introCards = $('.card--type-1')
    $introCards.each(function (index, elem) {
      gsap.to(elem, {duration: .5, opacity: 1, delay: (index / 4) + 5.5})
    })
    var $timeCards = $('.card--time')
    $timeCards.each(function (index, elem) {
      gsap.to(elem, {duration: .5, opacity: 1, delay: index + 6.5})
    })
    ...
    gsap.to($('.container-background'), {duration: 1, opacity: 1, delay: 10})
    var $gapCards = $('.card--type-7')
    $gapCards.each(function (index, elem) {
      var delay = 11 + (Math.random() * 2)
      var direction = Math.floor(Math.random() * 4)
      var $gapCard = $(elem)
